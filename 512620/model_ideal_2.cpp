...
int main() {
    ...
    // Print using std::for_each
    std::cout << "Deque elements: ";
    std::for_each(deque.begin(), deque.end(), [](int& val) {
        std::cout << val << " ";
    });
    std::cout << std::endl; // Should print 0 1 2

    // Insert one more element
    deque.insertBack(3);

    // Print again using std::for_each
    std::cout << "Deque elements after adding 3: ";
    std::for_each(deque.begin(), deque.end(), [](int& val) {
        std::cout << val << " ";
    });
    ...
    std::cout << "Back: " << deque.peekBack() << std::endl;   // Should print 3
    std::cout << "Size: " << deque.size() << std::endl;       // Should print 4

    std::cout << "Removed Front: " << deque.removeFront() << std::endl; // Should print 0
    std::cout << "Removed Back: " << deque.removeBack() << std::endl;   // Should print 3

    std::cout << "Size after removals: " << deque.size() << std::endl; // Should print 2
