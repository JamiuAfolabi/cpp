...
int shortest(const vector<vector<int>>& grid, int rows, int cols) {
    ...
    if constexpr (is_same_v<Container, set<Cell>>) {
        pq.insert({0, 0, grid[0][0]}); // For set: insert
    } else {
        pq.push({0, 0, grid[0][0]}); // For priority_queue: push
    }
    ...
    while (!pq.empty()) {
        // Get the cell with the lowest cost (for set, it's the first element, for priority_queue, top element)
        Cell current;
        if constexpr (is_same_v<Container, set<Cell>>) {
            current = *pq.begin();
            pq.erase(pq.begin());
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (isInsideGrid(newX, newY, rows, cols)) {
                ...
                if (newCost < dist[newX][newY]) {
                    ...
                    if constexpr (is_same_v<Container, set<Cell>>) {
                        pq.insert({newX, newY, newCost}); // For set: insert
                    } else {
                        pq.push({newX, newY, newCost}); // For priority_queue: push
                    }
                }
            }
        }
