...
// Define a template alias for the Cell Manager
#if USE_PRIORITY_QUEUE
using CellManager = priority_queue<Cell, vector<Cell>, greater<Cell>>;
#else
using CellManager = set<Cell>;
#endif
...
int shortest(const vector<vector<int>>& grid, int rows, int cols) {
    ...
    CellManager cellManager; // Use the defined CellManager alias
    cellManager.push({0, 0, grid[0][0]});
    ...
    while (!cellManager.empty()) {
        Cell current = cellManager.top();
        cellManager.pop();
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (isInsideGrid(newX, newY, rows, cols)) {
                ...
                // If the new cost is less than the recorded cost, update and push into the manager
                if (newCost < dist[newX][newY]) {
                    dist[newX][newY] = newCost;
                    // Push the new cell into the CellManager
                    cellManager.push({newX, newY, newCost});
                }
            }
        }
    }
