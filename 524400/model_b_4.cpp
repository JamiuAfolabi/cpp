...
public:
    ...
    std::vector<std::pair<int, int>> rangeQuery(int x1, int y1, int x2, int y2) const {
        ...
        for (const auto& entry : data) {
            ...
            if (x1 <= row && row <= x2 && y1 <= col && col <= y2) {
...
int main() {
    ...
    matrix.insert(9, 0, 3);
    matrix.insert(8, 3, 2);
    matrix.insert(10, 9, 9);
    matrix.insert(2, 7, 5);

    // Range query
    std::vector<std::pair<int, int>> elementsInRectangle = matrix.rangeQuery(0, 0, 10, 10);

    std::cout << "Non-zero elements in the rectangle [(0, 0), (10, 10)]:" << std::endl;
    for (const auto& element : elementsInRectangle) {
        std::cout << "Element at (" << element.first << ", " << element.second << ")" << std::endl;
    }
