...
private:
    ...
    bool eofReached; // Flag to check if EOF is reached
    500ms lf; // Simulated delay for reading
...
public:
    ...
    std::future<bool> readRecordAsync(std::vector<std::string>& fields) {
        return std::async([this, &fields]() {
...
private:
    void readNextChunk() {
        ...
        char chunk[8192]; // Size can be adjusted based on performance needs
...
void readRecords(StreamHandler& handler, int threadId) {
    std::vector<std::string> fields;
    
    while (handler.readRecordAsync(fields).get()) {
        std::cout << "Thread " << threadId << " read: ";
...
int main() {
    try {
        ...
        // Create several threads to read concurrently
        std::vector<std::future<void>> futures;
        for (int i = 0; i < std::thread::hardware_concurrency(); ++i) {
            futures.push_back(std::async(std::launch::async, readRecords, std::ref(handler), i));
        }
