...
private:
    ...
    std::unique_ptr<char[]> createCopy(const char* str) const {
...
public:
    ...
    const std::shared_ptr<const char[]> getSharedMessage() const {
        std::lock_guard<std::mutex> lock(mutex_);
        if (message_) {
            return std::make_shared<const char[]>(message_.get());
        }
        return nullptr;
    }
...
private:
    ...
    std::unordered_map<std::string, std::shared_ptr<const char[]>> cache_;
    mutable std::mutex cache_mutex_;

    void cacheMessage(const StringHandler& msg) {
        std::lock_guard<std::mutex> lock(cache_mutex_);
        cache_[msg.getSenderID()] = msg.getSharedMessage();
        Logger::log("Cached message from sender: " + msg.getSenderID());
    }
...
public:
    void addMessage(const StringHandler& msg) {
        ...
        cacheMessage(msg);
        Logger::log("Added message to history and cached it");
    }
    ...
    std::shared_ptr<const char[]> getCachedMessage(const std::string& senderID) const {
        std::lock_guard<std::mutex> lock(cache_mutex_);
        auto it = cache_.find(senderID);
        if (it != cache_.end()) {
            Logger::log("Retrieved cached message from sender: " + senderID);
            return it->second;
        }
        Logger::log("Message not found in cache from sender: " + senderID);
        return nullptr;
    }
...
int main() {
    try {
        ...
        // Accessing cached messages
        auto cachedMsg1 = history.getCachedMessage("Sender1");
        if (cachedMsg1) {
            std::cout << "Retrieved cached message from Sender1: " << *cachedMsg1 << std::endl;
        }

        auto cachedMsg4 = history.getCachedMessage("Sender4");
        if (!cachedMsg4) {
            std::cout << "Message from Sender4 not found in cache." << std::endl;
        }
