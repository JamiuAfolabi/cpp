...
private:
    ...
    std::shared_ptr<char[]> createCopy(const char* str) {
        if (!str) return nullptr;
        size_t len = strlen(str) + 1;
        auto copy = std::make_shared<char[]>(len);
        strcpy(copy.get(), str);
...
public:
    ...
    void concat(const char* msg) {
        ...
        if (!message_) {
            ...
            auto new_message = std::make_shared<char[]>(current_len + add_len + 1);
...
// MessageHistory class with caching support
class MessageHistory {
private:
    std::vector<std::shared_ptr<StringHandler>> messages;
    mutable std::mutex mutex_;

public:
    void addMessage(const std::shared_ptr<StringHandler>& msg) {
        std::lock_guard<std::mutex> lock(mutex_);
        messages.push_back(msg);
        Logger::log("Added message to history");
    }

    void batchProcessMessages(const std::vector<std::shared_ptr<StringHandler>>& newMsgs) {
        std::lock_guard<std::mutex> lock(mutex_);
        messages.insert(messages.end(), newMsgs.begin(), newMsgs.end());
        Logger::log("Batch processed " + std::to_string(newMsgs.size()) + " messages");
    }

    std::vector<std::shared_ptr<StringHandler>> searchMessages(const std::string& keyword) const {
        std::lock_guard<std::mutex> lock(mutex_);
        std::vector<std::shared_ptr<StringHandler>> foundMessages;
    ...
    std::map<std::string, std::vector<std::shared_ptr<StringHandler>>> categorizeMessages() const {
        std::lock_guard<std::mutex> lock(mutex_);
        std::map<std::string, std::vector<std::shared_ptr<StringHandler>>> categories;
        ...
        Logger::log("Categorized messages into " + std::to_string(categories.size()) + " categories");
        return categories;
    }
...
int main() {
    try {
        ...
        auto msg1 = std::make_shared<StringHandler>("Hello, this is an urgent message!", "Sender1", 1);
        auto msg2 = std::make_shared<StringHandler>("Just some information for you", "Sender2", 2);
        auto msg3 = std::make_shared<StringHandler>("This is a general message", "Sender3", 3);
