    ...
    void toggleService(Service service, bool isEnabled) {
        switch (service) {
            case Service::DineIn:
                ...
                notifyStaff("Dine-In service has been " + std::string(isEnabled ? "enabled" : "disabled"));
                break;
            case Service::Takeaway:
                takeawayEnabled = isEnabled;
                notifyStaff("Takeaway service has been " + std::string(isEnabled ? "enabled" : "disabled"));
                break;
            case Service::Delivery:
                deliveryEnabled = isEnabled;
                notifyStaff("Delivery service has been " + std::string(isEnabled ? "enabled" : "disabled"));
                break;
            default:
                break;
        }
...
private:
    ...
    void notifyStaff(const std::string& message) {
        std::cout << "Notification to staff: " << message << std::endl;
    }
};
