...
private:
    ...
    // Helper method to initialize the graph
...
public:
    ...
    void detectArbitrage(const std::string& start, double initialCapital) {
        // Initialize the graph lazily if not already done
        initializeGraph();

        // Rest of the arbitrage detection logic remains the same
        const std::unordered_map<std::string, std::vector<Edge>>& graph = graphPtr->graph;
...
int main() {
    ...
    // Arbitrage detection is done using the Singleton instance
