...
int main() {
    ...
    // Deserialize from JSON with error handling and validation
    try {
        Stock newStock;
        newStock.DeserializeFromJson(serializedStock);

        CASSERT(newStock.symbol == "AAPL");
        CASSERT(newStock.price == 150.0);

        Option newOption;
        newOption.DeserializeFromJson(serializedOption);

        CASSERT(newOption.symbol == "AAPL");
        CASSERT(newOption.strikePrice == 155.0);
        CASSERT(newOption.expiryDate == "2023-12-01");

        Future newFuture;
        newFuture.DeserializeFromJson(serializedFuture);

        CASSERT(newFuture.commodity == "Gold");
        CASSERT(newFuture.contractSize == 100.0);

        std::cout << "Deserialized data verified successfully." << std::endl;
