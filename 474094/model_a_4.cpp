...
int main() {
    ...
    while (lineCount < 5) {
        ...
        if (!parsedInput.empty()) { // Write only non-empty parsed lines to the file
            // Truncate the parsedInput if it exceeds 20 characters using substr
            std::string truncatedInput = parsedInput.substr(0, 20); // Get first 20 characters

            outFile << truncatedInput << std::endl; // Save the line to the file
