...
public:
    ...
    static void printStats(const std::string& outputFilePath = "memory_stats.txt") {
        std::lock_guard<std::mutex> lock(statsMutex);
        ...
        report << "Total memory allocated: " << totalMem << " bytes\n"
        ...
        try {
            ...
            std::cout << "Memory usage statistics exported to: " << outputFilePath << std::endl;
        } catch (const std::exception& e) {
            std::cerr << "Error exporting memory statistics to file: " << e.what() << std::endl;
        }
    }
};
