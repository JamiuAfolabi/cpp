...
private:
    std::list<LoanApplication> applications;

public:
    void addApplication(const LoanApplication& application) {
        applications.push_back(application);
        std::make_heap(applications.begin(), applications.end(), LoanApplicationComparator());
    }

    void processApplications() {
        std::cout << "Processing Loan Applications...\n";
        while (!applications.empty()) {
            LoanApplication app = applications.front();
            if (isEligible(app)) {
            ...
            std::pop_heap(applications.begin(), applications.end(), LoanApplicationComparator());
        }
    }
