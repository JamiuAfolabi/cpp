...
public:
    ...
    // Function to update signals every second
    void updateSignals() {
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < cols; ++j) {
        ...
        checkCongestion(); // Check for congestion after updating
    }
...
int main() {
    ...
    for (int cyc = 0; cyc < 35; ++cyc) { // Run for 35 seconds
        std::cout << "Time: " << cyc << " seconds\n";
        trafficGrid.printStates();
        trafficGrid.updateSignals();
