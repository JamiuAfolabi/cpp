...
// Function to search for dishes by partial names
void searchSpecials(const vector<string>& specials, const string& query) {
    cout << "Search results for \"" << query << "\":" << endl;
    bool found = false;
    for (const auto& special : specials) {
        if (special.find(query) != string::npos) { // Check if query is in the special
            cout << "- " << special << endl;
            found = true;
        }
    }
...
int main() {
    ...
    // Search for a dish by partial name
    string searchTerm;
    cout << endl << "Enter a partial name to search for dishes: ";
    getline(cin, searchTerm);
    searchSpecials(dailySpecials, searchTerm);
